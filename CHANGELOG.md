...
## [2026-05-19] — Stabilita test E2E export-bundle su latenza WMS

### Changed
- [tests/test_e2e_p0_extended.py](tests/test_e2e_p0_extended.py) `test_export_bundle_endpoint_responds` ora gestisce `PlaywrightTimeoutError` con `pytest.skip` esplicito quando `/export-bundle` resta bloccato oltre timeout (latenza upstream WMS/backend).
- Timeout request test elevato a `120000ms` per ridurre falsi negativi in ambienti lenti.

### Validation
- `python -m pytest tests/test_e2e_p0_extended.py -k "test_export_bundle_endpoint_responds" -q` -> `1 skipped`
- `python -m pytest tests/test_e2e_p0_extended.py -q` -> `32 passed, 1 skipped`

## [2026-05-19] — Regressioni E2E pointer overlay (legend + export selection)

### Changed
- [tests/test_e2e_p0_extended.py](tests/test_e2e_p0_extended.py) aggiunti test:
  - `test_pointer_gauge_legend_it_en_labels` (verifica label legenda low/mid/high in IT/EN),
  - `test_pointer_panel_hides_during_export_selection_and_restores` (hide/show pannello coordinate durante export selection).
- [src/planimeter.js](src/planimeter.js) fix timing su `startSelectionExportMode`: refresh visibilita pannello coordinate subito dopo assegnazione `this.selectionExport`.

### Validation
- `python -m pytest tests/test_e2e_p0_extended.py -k "pointer_gauge_legend_it_en_labels or pointer_panel_hides_during_export_selection_and_restores" -q` -> `2 passed`
- `python -m pytest tests/test_e2e_p0_extended.py -q` -> `34 passed, 1 skipped`

## [2026-05-19] — Smoke visual pointer panel (desktop/mobile)

### Changed
- [tests/test_e2e_p0_extended.py](tests/test_e2e_p0_extended.py) aggiunti test visual smoke:
  - `test_pointer_panel_screenshot_desktop`
  - `test_pointer_panel_screenshot_mobile`
- I test catturano screenshot del solo pannello `#pointer-coordinates` e verificano bytes non-vuoti come guardrail rapido contro regressioni visuali grossolane.

### Validation
- `python -m pytest tests/test_e2e_p0_extended.py -k "pointer_panel_screenshot_desktop or pointer_panel_screenshot_mobile" -q` -> `2 passed`
- `python -m pytest tests/test_e2e_p0_extended.py -q` -> `36 passed, 1 skipped`

## [2026-05-19] — UX live coordinates + viewport/zoom gauges + cursori stato

### Changed
- [planimeter.html](planimeter.html) pannello `#pointer-coordinates` esteso con gauge live:
  - `Viewport X` (% posizione orizzontale nel viewport),
  - `Viewport Y` (% posizione verticale nel viewport),
  - `Zoom` (livello live + percentuale normalizzata).
- [styles.css](styles.css) nuovi stili per gauge coordinate e nuove classi cursore stato mappa (`map-cursor-*`).
- [src/planimeter.js](src/planimeter.js) coordinate live ora aggiornate anche fuori da Navigate, con policy anti-conflitto:
  - freeze/reset durante processi interattivi (`draw/measure`, `hole draw`, export selection, busy M3),
  - resume automatico al termine,
  - zoom gauge aggiornato su cambio risoluzione.
- [src/planimeter.js](src/planimeter.js) aggiunto mapper centralizzato cursore coerente con stato corrente (`navigate`, `draw`, `measure`, `edit`, `edit selection armed`, `delete`, `hole`, `busy`).
- [src/i18n/it.js](src/i18n/it.js), [src/i18n/en.js](src/i18n/en.js) aggiunte stringhe `coords.gaugePlaceholder`, `coords.viewportX`, `coords.viewportY`, `coords.zoom`.
- [tests/test_e2e_p0_extended.py](tests/test_e2e_p0_extended.py) aggiunti test E2E:
  - visibilita panel/gauge in Edit,
  - freeze panel durante draw in corso,
  - cambio classe cursore per stato tool.

### Validation
- `node --check src/planimeter.js`
- `node --check src/i18n/it.js`
- `node --check src/i18n/en.js`
- `python -m pytest tests/test_e2e_p0_extended.py -k "pointer_panel_visible_in_edit_and_gauges_populated or pointer_panel_freezes_during_draw_conflict or cursor_class_changes_by_mode or overlap_preview_and_merge_action or edit_selection_tool_preview_and_merge_action" -q`

## [2026-05-19] — Tool Selection esplicito in Edit per Merge/Subtract

### Changed
- [src/planimeter.js](src/planimeter.js) aggiunto tool esplicito `Selection Merge/Subtract` in Edit mode:
  - arming su feature base selezionata,
  - click su feature operanda in overlap (stesso layer),
  - apertura preview diff con azioni `Accept` / `Merge` / `Subtract` / `Reject` senza nuovo draw.
- [src/planimeter.js](src/planimeter.js) riuso del pannello report overlap anche per flusso selection (`operationType: edit-selection-overlap`) con gestione apply/reject dedicata.
- [planimeter.html](planimeter.html) aggiunto pulsante toolbar `#btn-edit-selection-overlap` nel blocco strumenti Edit.
- [src/i18n/it.js](src/i18n/it.js), [src/i18n/en.js](src/i18n/en.js) nuove stringhe tool/messaggi/report per selection overlap.
- [src/i18n/it.js](src/i18n/it.js), [src/i18n/en.js](src/i18n/en.js) testo effect overlap draw aggiornato da `CTRL` a `ALT`.
- [tests/test_e2e_p0_extended.py](tests/test_e2e_p0_extended.py) nuovo test deterministico `test_edit_selection_tool_preview_and_merge_action`.

### Validation
- `node --check src/planimeter.js`
- `node --check src/i18n/it.js`
- `node --check src/i18n/en.js`
- `python -m pytest tests/test_e2e_p0_extended.py -k "overlap_preview_and_merge_action or edit_selection_tool_preview_and_merge_action" -q`

## [2026-05-19] — Overlap draw preview con azioni boolean (fase 2)

### Changed
- [src/planimeter.js](src/planimeter.js) la guardia overlap in draw ora, con ALT premuto, apre un preview report dedicato con metriche/diff e azioni `Accept` / `Merge` / `Subtract` / `Reject`.
- [src/planimeter.js](src/planimeter.js), [planimeter.html](planimeter.html), [styles.css](styles.css) aggiunto warning flottante dedicato al Draw Hole:
  - se Draw Hole viene invocato senza target poligonale valido,
  - se il click destro parte fuori target ma viene usata la feature selezionata come fallback,
  - se il draft hole esce dal perimetro target (con reject).
- [src/planimeter.js](src/planimeter.js) aggiunte operazioni geometriche boolean su stesso layer:
  - `Accept`: mantiene il draft cosi com'e,
  - `Merge`: unione draft + feature in overlap (rimozione delle feature assorbite),
  - `Subtract`: mantiene solo la porzione non sovrapposta del draft,
  - `Reject`: rollback completo (rimozione draft).
- [planimeter.html](planimeter.html), [src/planimeter.js](src/planimeter.js) esteso il pannello report esistente con pulsanti `Merge` e `Subtract` visibili solo in preview overlap.
- [src/i18n/it.js](src/i18n/it.js), [src/i18n/en.js](src/i18n/en.js) aggiunte stringhe UI/messaggi per preview overlap e outcome azioni.
- [tests/test_e2e_p0_extended.py](tests/test_e2e_p0_extended.py) nuovo test deterministico `test_ctrl_overlap_preview_and_merge_action`.

### Validation
- `node --check src/planimeter.js`
- `node --check src/i18n/it.js`
- `node --check src/i18n/en.js`
- `python -m pytest tests/test_e2e_p0_extended.py -k "overlap_preview_and_merge_action|draw_hole" -q`

## [2026-05-19] — Interaction consistency (fase 1)

### Changed
- [src/ui/context-menu.js](src/ui/context-menu.js) ora usa anche la feature selezionata come fallback target nel context menu, così `Draw hole` resta disponibile in edit quando il click destro non intercetta pixel feature ma esiste una selezione valida.
- [src/planimeter.js](src/planimeter.js) in modalità `edit`, click sinistro su area non disegnata forza il ritorno a `navigate` (`msg.editClickEmptyNavigate`).
- [src/planimeter.js](src/planimeter.js) introdotto handler ESC globale:
  - annulla hole draw attivo,
  - annulla draw/measure in corso,
  - rifiuta preview pending,
  - annulla export selection attivo,
  - se nessun tool attivo e non in navigate: switch a navigate.
- [src/planimeter.js](src/planimeter.js) aggiunta guardia overlap in `drawend` sullo stesso layer: senza CTRL apre conferma Accept/Reject; su Reject rimuove il draft con rollback; con CTRL applica override rapido (keep-as-drawn).
- [src/i18n/it.js](src/i18n/it.js), [src/i18n/en.js](src/i18n/en.js) aggiunte stringhe `msg.editClickEmptyNavigate`, `msg.esc.toolCancelled`, `msg.drawOverlap*` e `confirm.drawOverlap`.
